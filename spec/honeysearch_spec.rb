...
RSpec.describe Honeysearch do
  ...
  it 'parses a result correctly' do
    expect(Honeysearch::Parser.new(RESULT).results.first.artist).to eq 'Aqours'
    expect(Honeysearch::Parser.new(RESULT).results.first.title).to eq 'i-n-g, I TRY!!'
    expect(Honeysearch::Parser.new(RESULT).results.first.number).to eq '4570B6'
    expect(Honeysearch::Parser.new(RESULT).results.first.lyrics).to eq 'まるで夢のようなって'
    expect(Honeysearch::Parser.new(RESULT).results.first.tieup).to eq 'ラブライブ!サンシャイン!!The School Idol Movie Over the...<番組名>'
    expect(Honeysearch::Parser.new(RESULT).results.first.genre).to eq 'アニメ'
  end
end
